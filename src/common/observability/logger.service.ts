...
export class LoggerService implements NestLoggerService {
  ...
  info(message: string | object, context?: string, details?: any): void {
    if (typeof message === 'string') {
      ...
      this.winstonLogger.info((message as any).message || JSON.stringify(message), {
        context: context || this.getContext(),
        ...((message as any).details && {details: (message as any).details})
      });
    }
  }
  ...
  warn(message: string | object, context?: string, stack?: string | any, details?: any): void {
    ...
    if (typeof message === 'object' && message !== null && !(message instanceof Error)) {
      const warnMessage = (message as any).message || JSON.stringify(message);
      const warnDetails = details || (message as any).details;

      this.winstonLogger.warn(warnMessage, {
        context: context || this.getContext(),
        ...(typeof stack === 'string' && {stack}),
        ...(warnDetails && {details: warnDetails}),
        ...(typeof stack === 'object' && !details && {details: stack})
      });
      return;
    }
    ...
    const warnMessage = typeof message === 'string' ? message : message instanceof Error ? message.message : String(message);
    const warnStack = typeof stack === 'string' ? stack : undefined;
    const warnDetails = typeof stack === 'object' ? stack : details;
  ...
  error(message: string | object, context?: string, stack?: string | any, details?: any): void {
    ...
    if (typeof message === 'object' && message !== null && !(message instanceof Error)) {
      ...
      const errorDetails = details || (message as any).details;
    ...
    const errorMessage = typeof message === 'string' ? message : message instanceof Error ? message.message : String(message);
    const errorStack = typeof stack === 'string' ? stack : undefined;
    const errorDetails = typeof stack === 'object' ? stack : details;
  ...
  private getContext(depth: number = 4): string {
    try {
      ...
      // Extract method name: at ClassName.methodName (file:line:column)
